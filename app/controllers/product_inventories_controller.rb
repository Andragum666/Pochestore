class ProductInventoriesController < ApplicationController
  before_action :set_product_inventory, only: [:show, :edit, :update, :destroy]

  # GET /product_inventories
  # GET /product_inventories.json
  def index
    @product_inventories = ProductInventory.all
  end

  # GET /product_inventories/1
  # GET /product_inventories/1.json
  def show
  end
  ...
  # POST /product_inventories.json
  def create
    @product_inventory = ProductInventory.new(product_inventory_params)

    respond_to do |format|
      if @product_inventory.save
        format.html { redirect_to @product_inventory, notice: 'Product inventory was successfully created.' }
  ...
  # PATCH/PUT /product_inventories/1.json
  def update
    respond_to do |format|
      if @product_inventory.update(product_inventory_params)
        format.html { redirect_to @product_inventory, notice: 'Product inventory was successfully updated.' }
  ...
  # DELETE /product_inventories/1.json
  def destroy
  ...
  private
    ...
    def product_inventory_params
      params.require(:product_inventory).permit(:inventory_id, :product_id, :quantity)
    end
end
